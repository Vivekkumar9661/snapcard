...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        if (!assignment) {
            console.log(`[API:Accept] findById failed for ${id}. Trying findOne by string and ObjectId...`);
            assignment = await DeliveryAssignment.findOne({
                $or: [{ _id: id as any }, { _id: (mongoose.Types.ObjectId.isValid(id) ? new mongoose.Types.ObjectId(id) : id) as any }]
            });
        }

        if (!assignment) {
            console.log(`[API:Accept:v2] NOT FOUND: Assignment ID ${id} is missing from deliveryassignments collection.`);
            return NextResponse.json({ message: `[V2] Assignment ${id} not found in database. Please refresh.` }, { status: 404 });
        }
        ...
        if (currentStatus !== "broadcasted" && currentStatus !== "brodcasted") {
            ...
            return NextResponse.json({ message: `[V2] Conflict: Assignment is already ${currentStatus}` }, { status: 409 });
        }

        const bTo = (assignment as any).broadcastedTo || (assignment as any).brodcastedTo || [];
        const broadcastedTo = Array.isArray(bTo) ? bTo.map((bId: any) => bId.toString()) : [];
        const deliveryBoyObjectIdStr = new mongoose.Types.ObjectId(deliveryBoyID).toString();

        console.log(`[API:Accept:v2] BROADCAST LIST: ${JSON.stringify(broadcastedTo)}`);
        console.log(`[API:Accept:v2] CHECKING USER: ${deliveryBoyID} OR ${deliveryBoyObjectIdStr}`);

        if (!broadcastedTo.includes(deliveryBoyID) && !broadcastedTo.includes(deliveryBoyObjectIdStr)) {
            console.log(`[API:Accept:v2] UNAUTHORIZED: User not in broadcast list`);
            return NextResponse.json({ message: `[V2] Error: You [${deliveryBoyID}] are not in the broadcast list for this order.` }, { status: 403 });
        }
        ...
        if (!order) {
            ...
            return NextResponse.json({ message: "[V2] Order linked to assignment not found" }, { status: 404 });
        }
